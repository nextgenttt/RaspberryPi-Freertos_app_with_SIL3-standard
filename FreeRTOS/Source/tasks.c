 ...
 */
#define prvGetTCBFromHandle( pxHandle ) ( ( tskTCB * ) ( pxHandle ) )
...
{
signed portBASE_TYPE xReturn = 0x0;;
tskTCB * pxNewTCB;

	if(pxTaskCode == NULL)
        {
            bcm2835_miniuart_sendstr("Scheduler Error: Null Parameter passed as an argument\r\n");
            bcm2835_miniuart_sendstr("               : Returning errTASK_CREATE_NULL_PARAMETER_SUPPLIED\r\n");
           xReturn = OS_ERROR_WARNING(OS_ERROR_COMPONENT_TASKS,errTASK_CREATE_NULL_PARAMETER_SUPPLIED);
           return xReturn;
        }
	...
	else
	{
           ...
           xReturn = OS_ERROR_WARNING(OS_ERROR_COMPONENT_TASKS,errTASK_CREATE_COULD_NOT_ALLOCATE_REQUIRED_MEM);
           return xReturn;
	}
...
__attribute__((no_instrument_function))
	...
	unsigned portBASE_TYPE uxReturn;
                if(pxTask == NULL)
                {
                     uxReturn = OS_ERROR_WARNING(OS_ERROR_COMPONENT_TASKS,errGET_PRIORITY_NULL_PARAMETER_SUPPLIED);
		     return uxReturn;
                }
		taskENTER_CRITICAL();
		{
...
portBASE_TYPE xReturn = 0;
        ...
        {
          bcm2835_miniuart_sendstr("Scheduler Error : Scheduler is already runnning\r\n");
          bcm2835_miniuart_sendstr("Scheduler Return value : OS_ERROR_COMPONENT_TASKS | errSCHEDULER_ALREADY_RUNNING\r\n");
          return OS_ERROR_WARNING(OS_ERROR_COMPONENT_TASKS,errSCHEDULER_ALREADY_RUNNING);
        }
        if ((listLIST_IS_EMPTY( ( xList * ) &pxDelayedTaskList ) == pdTRUE) &&
            (listLIST_IS_EMPTY( ( xList * ) &pxReadyTasksLists ) == pdTRUE)&&
            (listLIST_IS_EMPTY( ( xList * ) &pxOverflowDelayedTaskList ) == pdTRUE)&&
            (listLIST_IS_EMPTY( ( xList * ) &xTasksWaitingTermination ) == pdTRUE)&&
            (listLIST_IS_EMPTY( ( xList * ) &xSuspendedTaskList ) == pdTRUE))
        {
                   bcm2835_miniuart_sendstr("Scheduler Error : Ready/Waiting/Suspended/Overflow lists are empty. Scheduler not running\r\n");
                   bcm2835_miniuart_sendstr("Scheduler Return value : OS_ERROR_COMPONENT_TASKS | errNO_TASKS_CREATED\r\n");
                   return OS_ERROR_WARNING(OS_ERROR_COMPONENT_TASKS,errNO_TASKS_CREATED);
        }
